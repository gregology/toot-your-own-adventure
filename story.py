...
class Story:
    def __init__(self, api_key:str, organization:str, genre:str="sci fi", poll_character_limit:int=50, number_of_cues:int=3):
    ...
    def summarize(self, text:str, character_limit:int=50)->str:
        ...
        output = re.sub(r'\s*\(\d+\s*characters\)', '', response)
    ...
    def prompt(self):
        ...
        while True:
            ...
            if choice in possible_choices:
                break
            print(f"Invalid choice, please choose {', '.join(possible_choices)} or type 'q' to quit")
